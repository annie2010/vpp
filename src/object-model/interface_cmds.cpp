...
interface::AFPacketdelete_cmd::AFPacketdelete_cmd(HW::item<handle_t> &item,
                                                  const std::string &name):
    delete_cmd(item, name)
{
}

rc_t interface::AFPacketdelete_cmd::issue(connection &con)
{
    msg_t req(con.ctx(), std::ref(*this));
...
std::string interface::AFPacketdelete_cmd::to_string() const
{
    std::ostringstream s;
